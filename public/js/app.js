...
/***/ (() => {
    ...
    $(function () {
      $('#btn-addInstructor2').on('click', function(e) {
        ...
        if($.isNumeric(codpes)){
          $.ajax({
            url: baseURL + '/docente/consulta?codpes=' + codpes,
            dataType: 'json',
          success: function success(instructorName){
            var nompes = instructorName;
            if(nompes != ""){
              var html = ['<div id="instrutor">',
                  '<input id="docente" name="codpes" type="hidden" value='+codpes+'>',
                  '<label id="label-instrutor" class="font-weight-normal">'+nompes+'</label>',
                  '<a class="btn btn-link btn-sm text-dark text-decoration-none"',
                  '    style="padding-left:0px"',
                  '    id="btn-remove-instrutor"',
              ...
              document.getElementById("btn-addDocente").disabled = true;
            } else{
